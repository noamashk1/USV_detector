...
def validate_config(
    ...
    if not (0.0 < config.background_alpha <= 1.0):
        raise ValueError("background_alpha must be in range (0, 1]")
...
class MazeSetupGUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(btn_row, text="Load settings from JSON", command=self._on_load_json).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(btn_row, text="Interactive setup", command=self._on_interactive).pack(
            side=tk.LEFT, padx=4
        )
        ...
        ttk.Checkbutton(
            ...
            variable=self.use_adaptive_threshold_var,
        ...
        ttk.Label(action_row, textvariable=self.status_var, foreground="#333").pack(anchor="w", fill=tk.X)
    ...
    def _sync_params_to_ui(self) -> None:
        ...
        self.background_alpha_var.set(str(self.config.background_alpha))
        self.min_consecutive_frames_var.set(str(self.config.min_consecutive_frames))
        self.cooldown_s_var.set(str(self.config.cooldown_s))
        self.morph_kernel_size_var.set(str(self.config.morph_kernel_size))
        self.morph_open_iterations_var.set(str(self.config.morph_open_iterations))
        self.morph_close_iterations_var.set(str(self.config.morph_close_iterations))
        self.adaptive_k_sigma_var.set(str(self.config.adaptive_k_sigma))
    ...
    def _apply_params_from_ui(self) -> None:
        try:
            ...
            raise ValueError("Invalid parameter value type") from exc
...
def init_zone_runtime(zones: list[RoiZone], gray_frame: np.ndarray, blur_kernel: int) -> None:
    ...
    for zone in zones:
        ...
        zone.background = cv2.GaussianBlur(roi, k, 0).astype(np.float32)
...
def process_zone(zone: RoiZone, gray: np.ndarray, config: MazeConfig) -> tuple[bool, np.ndarray, float]:
    ...
    bg_u8 = cv2.convertScaleAbs(zone.background)
    diff = cv2.absdiff(bg_u8, roi_blurred)

    if config.use_adaptive_threshold:
        sigma = float(np.std(diff))
        thr_val = max(float(config.diff_threshold), config.adaptive_k_sigma * sigma)
        mask = cv2.threshold(diff, thr_val, 255, cv2.THRESH_BINARY)[1]
    else:
        mask = cv2.threshold(diff, config.diff_threshold, 255, cv2.THRESH_BINARY)[1]
...
def run_monitoring(cap: cv2.VideoCapture, config: MazeConfig, first_frame: np.ndarray) -> None:
    ...
    init_zone_runtime(config.zones, first_gray, config.blur_kernel)

    gpio: GpioManager | None = None
    try:
        gpio = GpioManager(pins=[z.gpio for z in config.zones], pulse_width_s=config.pulse_width_s)
    ...
    try:
        while True:
            ...
            for zone in config.zones:
                raw_motion, mask, motion_amount = process_zone(zone, gray, config)
                ...
                debug[zone.name] = f"amt:{motion_amount:.0f} cnt:{zone.consecutive_count} cd:{cooldown_left:.1f}s"
    ...
    try:
        while True:
            ...
            for zone in config.zones:
                raw_motion, mask, motion_amount = process_zone(zone, gray, config)
                if raw_motion:
                    zone.consecutive_count += 1
                else:
                    zone.consecutive_count = 0

                cooldown_left = max(0.0, config.cooldown_s - (now - zone.last_trigger_time))
                ready = cooldown_left <= 0.0
                trigger = zone.consecutive_count >= config.min_consecutive_frames and ready
                if trigger:
                    if gpio is not None:
                        gpio.pulse_async(zone.gpio)
                    zone.last_trigger_time = now
